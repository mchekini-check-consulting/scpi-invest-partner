...
//     }
    ...
    stage("Archive Jar files"){
        archiveArtifacts artifacts: 'target/scpi-invest-partner-*.jar', followSymlinks: false
    }

    stage("Build docker Image"){
        sh "sudo docker build -t mchekini/scpi-invest-partner:$GIT_COMMIT_HASH ."
    }
    ...
    stage("Push Docker Image"){
        withCredentials([usernamePassword(credentialsId: 'mchekini-docker-hub', passwordVariable: 'password', usernameVariable: 'username')]) {
            ...
            sh "sudo docker push mchekini/scpi-invest-partner:$GIT_COMMIT_HASH"
            sh "sudo docker rmi mchekini/scpi-invest-partner:$GIT_COMMIT_HASH"
        }
    }
}